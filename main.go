...
func main() {
	...
	check(err)
	oneHundredBaseUnits := big.NewInt(cfg.TestAmount)
	buyPrice, err := exchange.GetEthToTokenOutputPrice(nil, oneHundredBaseUnits)
	check(err)
	sellPrice, err := exchange.GetTokenToEthInputPrice(nil, oneHundredBaseUnits)
	check(err)
	fmt.Printf("UNISWAP PRICES for %v token(s)\n", cfg.TestAmount)
	fmt.Printf("- price to buy 1 gas tokens on uniswap (in wei):\t%s\n", buyPrice.String())
	fmt.Printf("- price to sell 1 gas tokens on uniswap (in wei):\t%s\n", sellPrice.String())
	fmt.Println()
